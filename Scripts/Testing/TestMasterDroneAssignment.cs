using System;
using Guribo.FPVDrones.Scripts.Testing;
...
{
    public class TestMasterDroneAssignment : UdonSharpBehaviour
    {
        ...
        private void InitializeTest()
        {
            ...
            {
                Debug.Log($"{LogPrefix} {name}.InitializeTest: Not master, nothing to do",
                    this);
        ...
        private void Update()
        {
            ...
            {
                Debug.LogError($"{LogPrefix} {name}.RunTest: Master is supposed to have exactly 1 drone " +
        ...
        private void RunTest()
        {
            if (!Networking.IsMaster)
            {
                Debug.Log($"{LogPrefix} {name}.RunTest: Not master, nothing to do",
                    this);
                testController.TestCompleted(true);
                return;
            }
        ...
        private void CleanUpTest()
        {
            if (!Networking.IsMaster)
            {
                Debug.Log($"{LogPrefix} {name}.CleanUpTest: Not master, nothing to do",
                    this);
